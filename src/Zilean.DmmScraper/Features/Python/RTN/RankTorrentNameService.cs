...
public class RankTorrentNameService
{
    ...
    public RankTorrentNameService(PythonEngineService pythonEngineService)
    {
    ...
    public List<ParseTorrentTitleResponse?> BatchParse(IReadOnlyCollection<string?> torrents, int chunkSize = 500, int workers = 20, bool trashGarbage = true, bool logErrors = false, bool throwOnErrors = false)
    {
        ...
        try
        {
            ...
            using var gil = Py.GIL();
            var pythonList = new PyList(torrents.Select(x => new PyString(x).As<PyObject>()).ToArray());
